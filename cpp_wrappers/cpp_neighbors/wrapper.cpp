...
// Definition of the batch_neighbors method
// **********************************
...
static PyObject* batch_neighbors(PyObject* self, PyObject* args, PyObject* keywds)
{
    ...
    // Parse the input  
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "OOOO|$f", kwlist, 
                                      &queries_obj, &supports_obj, &q_batches_obj, &s_batches_obj, &radius))
    {
    ...
    // Check that the input arrays respect the dims
    ...
    if ((int)PyArray_NDIM((PyArrayObject*)supports_array) != 2 ||
        (int)PyArray_DIM((PyArrayObject*)supports_array, 1) != 3)
    {
        Py_XDECREF(queries_array);
        Py_XDECREF(supports_array);
        Py_XDECREF(q_batches_array);
    ...
    // Convert PyArray to C++ vectors
    vector<PointXYZ> queries((PointXYZ*)PyArray_DATA((PyArrayObject*)queries_array),
                             (PointXYZ*)PyArray_DATA((PyArrayObject*)queries_array) + Nq);
    vector<PointXYZ> supports((PointXYZ*)PyArray_DATA((PyArrayObject*)supports_array),
                              (PointXYZ*)PyArray_DATA((PyArrayObject*)supports_array) + Ns);
    vector<int> q_batches((int*)PyArray_DATA((PyArrayObject*)q_batches_array),
                          (int*)PyArray_DATA((PyArrayObject*)q_batches_array) + Nb);
    vector<int> s_batches((int*)PyArray_DATA((PyArrayObject*)s_batches_array),
                          (int*)PyArray_DATA((PyArrayObject*)s_batches_array) + Nb);

    // Create result container
    vector<int> neighbors_indices;

    // Compute results using your neighbors function (uncomment the correct one)
    ...
    // Maximal number of neighbors per query point
    int max_neighbors = neighbors_indices.size() / Nq;

    // Define output array dimensions
    npy_intp neighbors_dims[2] = { (npy_intp)Nq, (npy_intp)max_neighbors };

    // Create output array
    PyObject* res_obj = PyArray_SimpleNew(2, neighbors_dims, NPY_INT);
    PyObject* ret = NULL;

    // Fill output array with the computed neighbor indices
    ...
    // Return the result as a new reference
